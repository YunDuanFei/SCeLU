...
def plot_2d_contour(surf_file, surf_name='train_loss', vmin=0.1, vmax=10, vlevel=0.5, show=False):
    ...
    CS = plt.contour(X, Y, Z, 25, cmap='summer')
    ...
    surf = ax.plot_surface(X, Y, Z, cmap='summer', linewidth=0, antialiased=False)
    plt.colorbar(surf, shrink=0.5, aspect=20)
    ax.set_xticks(np.arange(X.min(), X.max(), (X.max()-X.min())/4))
    ax.set_yticks(np.arange(Y.min(), Y.max(), (Y.max()-Y.min())/4))
    ...
    surf = ax.plot_surface(X, Y, Z, cmap='summer', linewidth=0, antialiased=False)
    plt.colorbar(surf, shrink=0.5, aspect=20)
    # CS = ax.contour(X, Y, Z, cmap='summer', offset=Z.max(), levels=np.arange(vmin, vmax, vlevel))
    CS = ax.contour(X, Y, Z, 25, cmap='summer', offset=Z.max())
    plt.clabel(CS, inline=1, fontsize=8)
    ax.set_xticks(np.arange(X.min(), X.max(), (X.max()-X.min())/4))
    ax.set_yticks(np.arange(Y.min(), Y.max(), (Y.max()-Y.min())/4))
    fig.savefig(surf_file + '_' + surf_name + '_3dcontour_surface.pdf', dpi=300,
                bbox_inches='tight', format='pdf')
...
def plot_contour_trajectory(surf_file, dir_file, proj_file, surf_name='loss_vals',
    ...
    CS1 = plt.contour(X, Y, Z, 25)
    CS2 = plt.contour(X, Y, Z, 25)
