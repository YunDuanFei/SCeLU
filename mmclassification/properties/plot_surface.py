...
def setup_surface_file(args, surf_file, dir_file):
    ...
    step = (args.xmax-args.xmin)/(args.xnum-1)
    xcoordinates = np.arange(args.xmin, args.xmax+step, step)
    f['xcoordinates'] = xcoordinates

    if args.y:
        ycoordinates = np.arange(args.ymin, args.ymax+step, step)
...
def crunch(surf_file, net, w, s, d, landspace_dataloader, loss_key, acc_key, comm, rank, args):
    """
    ...
    for count, ind in enumerate(inds):
        ...
        loss, acc = evaluation.eval_loss(net, criterion, landspace_dataloader, args.cuda)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data_split', default=1, type=int, help='the number of splits for the landspace_dataloader')
    parser.add_argument('--split_idx', default=0, type=int, help='the index of data splits for the landspace_dataloader')
    parser.add_argument('--trainloader', default='', help='path to the landspace_dataloader with random labels')
    ...
    parser.add_argument('--vlevel', default=0.1, type=float, help='plot contours every vlevel')
    ...
    #--------------------------------------------------------------------------
    net = model_loader.land_load(args.dataset, args.model, args.model_file)
    ...
    trainloader, testloader = landspace_dataloader.load_dataset(args.dataset, args.datapath,
