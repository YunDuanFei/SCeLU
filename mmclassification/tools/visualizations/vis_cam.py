...
def main():
    ...
    if os.path.isdir(args.img):
        for file in os.listdir(args.img):
            file_name = os.path.join(args.img, file)
            data, src_img = apply_transforms(file_name, cfg.data.test.pipeline)
            data_src_img.append([data, src_img, file_name])
    elif os.path.isfile(args.img):
        data, src_img = apply_transforms(args.img, cfg.data.test.pipeline)
        data_src_img.append([data, src_img])
    ...
    for d_s in tqdm(data_src_img):
        data, src_img, file_name = d_s[0], d_s[1], d_s[2]
        grayscale_cam = cam(
            data['img'].unsqueeze(0),
            targets,
            eigen_smooth=args.eigen_smooth,
            aug_smooth=args.aug_smooth)
        model_and_acfun = Path(args.config).stem.split('_')
        model_name, acfun_name = model_and_acfun[0], model_and_acfun[-1]
        img_path = Path(file_name)
        save_path = os.path.join(img_path.parent.parent, model_name, acfun_name, args.target_layers[0], img_path.name)
        show_cam_grad(grayscale_cam, src_img, title=args.method, out_path=save_path)
